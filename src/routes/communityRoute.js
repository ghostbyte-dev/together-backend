...
const deleteOrAcceptRequest = async (req, res, accept) => {
  ...
  if (!community) {
    resSend(res, null, ResStatus.ERROR, 'No Admin of a Community');
    return;
  }
  if (community.request.length === 0) {
    resSend(res, null, ResStatus.ERROR, 'No request from this user');
    return;
  }
  if (accept) {
...
router.get('/getbycode/:code', auth, async (req, res) => {
  ...
  if (!community) {
    resSend(
      ...
      null,
      ResStatus.ERROR,
      'Comunity with the id ' + code.toString() + " doesn't exist",
    );
    return;
  }
  resSend(res, {
    ...
    name: community.name,
    code: community.code,
    admin: community.user_community_fk_admin_idTouser,
    userCount: community._count.user,
  });
});
...
router.post('/create', auth, async (req, res) => {
  ...
  resSend(res, { code: inviteCode, id: community.id });
});
